...
interface TrainerPhotoManagerProps {
  trainerId: number;
  initialPhotos: Photo[];
}

// Mirrors packages/admin/src/app/branches/photos/branch-photo-manager.tsx.
// Embedded inline in each trainer card so the admin doesn't need a separate
// "photos" page for trainers — everything for one trainer lives in one place.
export function TrainerPhotoManager({ trainerId, initialPhotos }: TrainerPhotoManagerProps) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const photoRes = await fetch('/api/trainer-photos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ trainerId, imageUrl: url }),
      });
  ...
  async function handleDrop(targetIndex: number) {
    ...
    try {
      await fetch('/api/trainer-photos/reorder', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100">
      <div className="flex items-baseline justify-between mb-3">
        <p className="text-xs font-medium text-gray-600">Фотографии ({photos.length})</p>
        <p className="text-[10px] text-gray-400">Перетащите для сортировки. Первое — главное.</p>
      </div>
